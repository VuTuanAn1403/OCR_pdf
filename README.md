...
### 1. Báo lỗi `No module named 'pkg_resources'`
Lỗi xảy ra khi môi trường ảo mới sử dụng bản `setuptools >= 70`. Khắc phục bằng cách cài đặt đúng phiên bản:
```powershell
pip install "setuptools>=65.5.1,<70.0.0"
```

### 2. Tải mô hình VietOCR bị chậm hoặc timeout
Mô hình `vgg_seq2seq` được VietOCR tự động tải từ Google Drive trong lần chạy đầu tiên. Nếu mạng chặn hoặc tải chậm, bạn có thể tải file `vgg_seq2seq.pth` đặt vào thư mục tạm `%TEMP%` trên Windows hoặc `/tmp` trên Linux.

### 3. Thiếu công cụ build C++ trên Windows
Các gói `opencv-python-headless` và `onnxruntime` đều cung cấp sẵn binary wheel tương thích cho Python 3.10, 3.11, 3.12 64-bit. Hãy đảm bảo bạn cài đặt bản Python 64-bit chính thức.

---

## 13. Giới Hạn Đã Biết (Known Limitations)

- **Biểu đồ vector không chứa text**: Các biểu đồ dạng hình vẽ vector phức tạp không có nhãn chữ sẽ được bỏ qua trong file Markdown kết quả.
- **Bảng số liệu trải dài qua nhiều trang không lặp lại tiêu đề**: Các bảng nối tiếp nhau qua hơn 3 trang có thể được phân đoạn thành các khối bảng độc lập theo từng trang.
- **Chiều xoay văn bản**: Phiên bản V3.1 xử lý tối ưu nhất cho các trang có hướng đọc xuôi ($0^\circ$).

---

## 14. Bản Quyền & Thông Báo Bên Thứ Ba

Dự án có sử dụng và liên kết với các thư viện nguồn mở: PyMuPDF (AGPLv3/Commercial), VietOCR (MIT), RapidOCR (Apache 2.0), OpenCV (Apache 2.0), PyTorch (BSD-3). Chi tiết giấy phép vui lòng xem tại [THIRD_PARTY_NOTICES.md](THIRD_PARTY_NOTICES.md).
